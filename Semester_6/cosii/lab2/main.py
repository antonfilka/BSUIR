...
def main():
    ...
    function_values_1 = list(map(lambda x: np.sin(2 * x), arguments))
    function_values_2 = list(map(lambda x: np.cos(7 * x), arguments))
    ...
    ax1.set(title='sin(2x)')
    ax1.grid()

    ax2.plot(arguments, function_values_2)
    ax2.set(title='cos(7x)')
    ax2.grid()
